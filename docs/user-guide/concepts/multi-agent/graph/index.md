...
You can use a [`Graph`](/docs/api/python/strands.multiagent.graph#Graph) or [`Swarm`](/docs/api/python/strands.multiagent.swarm#Swarm) as a node within another Graph:
...
Graphs support remote A2A agents as nodes through the [`A2AAgent`](/docs/user-guide/concepts/multi-agent/agent-to-agent/index.md#a2aagent-consuming-remote-agents) class. You can add it directly to a graph just like a local agent. This enables distributed architectures where orchestration happens locally while specialized tasks run on remote services.
...
You can create custom node types by extending [`MultiAgentBase`](/docs/api/python/strands.multiagent.base#MultiAgentBase) to implement deterministic business logic, data processing pipelines, and hybrid workflows.
...
Graphs support multi-modal inputs like text and images using [`ContentBlocks`](/docs/api/python/strands.types.content#ContentBlock):
...
You can also execute a Graph asynchronously by calling the [`invoke_async`](/docs/api/python/strands.multiagent.graph#Graph.invoke_async) function:
...
Graphs support real-time streaming of events during execution using [`stream_async`](/docs/api/python/strands.multiagent.graph#Graph.stream_async). This provides visibility into node execution, parallel processing, and nested multi-agent systems.
...
When a Graph completes execution, it returns a [`GraphResult`](/docs/api/python/strands.multiagent.graph#GraphResult) object with detailed information:
...
Agents can dynamically create and orchestrate graphs by using the `graph` tool available in the [Strands tools package](/docs/user-guide/concepts/tools/community-tools-package/index.md).
