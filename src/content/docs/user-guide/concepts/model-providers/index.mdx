...
export const modelProviders = allDocs
  .filter(
    (doc) =>
  ...
  .map((doc) => {
    ...
    if (languages) {
      ...
      if (langArray.length === 1 && langArray[0] === 'Python') typescriptSupport = false
      if (langArray.length === 1 && langArray[0] === 'TypeScript') pythonSupport = false
    }
    const sidebarLabel = doc.data.sidebar?.label
    return {
      id: doc.id,
...
export const officialProviders = modelProviders.filter((p) => !p.community)
export const communityProviders = modelProviders.filter((p) => p.community)
