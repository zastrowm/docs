...
| Type | Description | | --- | --- | | `Iterable[dict[str, Any]]` | An iterable of response events from the Bedrock model |

Raises:

| Type | Description | | --- | --- | | `ContextWindowOverflowException` | If the input exceeds the model's context window. | | `EventStreamError` | For all other Bedrock API errors. |
