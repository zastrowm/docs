...
You can also use remote agents as tools through the [Agent-to-Agent (A2A) protocol](/user-guide/concepts/multi-agent/agent-to-agent/index.md). The `A2AAgent` class lets you wrap a remote A2A-compatible agent as a tool in your orchestrator, following the same pattern described above but communicating over the network. See [A2AAgent as a Tool](/user-guide/concepts/multi-agent/agent-to-agent/index.md#as-a-tool) for details.
