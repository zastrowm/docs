...
The `A2AAgent` class integrates with Strands multi-agent patterns that support it. Currently, you can use remote A2A agents in [Graph](/docs/user-guide/concepts/multi-agent/graph/index.md) workflows and as [tools in an orchestrator agent](#as-a-tool).
...
`A2AAgent` is not currently supported in Swarm patterns. Swarm coordination relies on tool-based handoffs that require capabilities not yet available in the A2A protocol. Use [Graph](/docs/user-guide/concepts/multi-agent/graph/index.md) workflows for multi-agent patterns with remote A2A agents.
...
A2A agents can be wrapped as tools within your agent’s toolkit, similar to the [Agents as Tools](/docs/user-guide/concepts/multi-agent/agents-as-tools/index.md) pattern but leveraging the A2A protocol for cross-platform communication.
