...
The [`Swarm`](/api/python/strands.multiagent.swarm#Swarm) constructor allows you to control the behavior and safety parameters:
...
Swarms support multi-modal inputs like text and images using [`ContentBlocks`](/api/python/strands.types.content#ContentBlock):
...
You can also execute a Swarm asynchronously by calling the [`invoke_async`](/api/python/strands.multiagent.swarm#Swarm.invoke_async) function:
...
Swarms support real-time streaming of events during execution using [`stream_async`](/api/python/strands.multiagent.swarm#Swarm.stream_async). This provides visibility into agent collaboration, handoffs, and autonomous coordination.
...
When a Swarm completes execution, it returns a [`SwarmResult`](/api/python/strands.multiagent.swarm#SwarmResult) object with detailed information:
...
Agents can dynamically create and orchestrate swarms by using the `swarm` tool available in the [Strands tools package](/user-guide/concepts/tools/community-tools-package/index.md).
