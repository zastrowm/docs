...
This Lambda deployment example does not implement response streaming as described in the [Async Iterators for Streaming](/user-guide/concepts/streaming/async-iterators/index.md) documentation. If you need streaming capabilities, consider using the [AWS Fargate deployment](/user-guide/deploy/deploy_to_aws_fargate/index.md) approach which does implement streaming responses.
...
When using [Model Context Protocol (MCP)](/user-guide/concepts/tools/mcp-tools/index.md) tools with Lambda, there are important considerations for connection lifecycle management.
