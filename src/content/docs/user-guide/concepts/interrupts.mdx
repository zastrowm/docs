...
```typescript
--8<-- "user-guide/concepts/interrupts.ts:hooks_example"
```
</Tab>
</Tabs>
...
- [`BeforeToolCallEvent`](@api/typescript/BeforeToolCallEvent) / [`BeforeToolsEvent`](@api/typescript/BeforeToolsEvent) — hook events that expose the ability to interrupt via the [`interrupt`](@api/typescript/InterruptParams) method
...
- [`AgentResult`](@api/typescript/AgentResult) — returned by `invoke()` / `stream()`, contains interrupt information when the agent pauses
    ...
    - `result.interrupts` — array of [`Interrupt`](@api/typescript/Interrupt) objects, each containing the user-provided `name` and `reason`, along with a unique `id`
- [`InterruptResponseContent`](@api/typescript/InterruptResponseContent) — content block type for resuming from an interrupt
...
In TypeScript, the tool callback receives a `context` parameter (the second argument) which provides the `interrupt` method.
...
Tool interrupts work similarly to hook interrupts with only a few notable differences:
...
- [`ToolContext`](@api/typescript/ToolContext) — the second argument passed to the tool callback, providing access to the `interrupt` method
    - You can learn more about `ToolContext` [here](./tools/custom-tools.md#toolcontext).
    - `context.interrupt({ name, reason? })` — halts the agent loop. `name` is a string identifier and `reason` is an optional JSON-serializable value. See [`InterruptParams`](@api/typescript/InterruptParams).
    - The `name` must be unique only among interrupt calls configured in the same tool definition. It is still advisable however to namespace your interrupts so as to more easily distinguish the calls when constructing responses outside the agent.
</Tab>
</Tabs>
