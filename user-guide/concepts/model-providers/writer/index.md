...
> **Note**: The `web_search` and `email_sender` tools in this example are custom tools that you would need to define. See [Python Tools](/user-guide/concepts/tools/custom-tools/index.md) for guidance on creating custom tools, or use existing tools from the [strands\_tools package](/user-guide/concepts/tools/community-tools-package/index.md).
...
> **Note**: Structured output disables streaming and returns the complete response at once, unlike regular chat completions, which stream by default. See [Callback Handlers](/user-guide/concepts/streaming/callback-handlers/index.md) for more details.
