...
TypeScript does not support callback handlers. For real-time event handling in TypeScript, use the [async iterator pattern](/user-guide/concepts/streaming/async-iterators/index.md) with `agent.stream()` or see [Hooks](/user-guide/concepts/agents/hooks/index.md) for lifecycle event handling.
...
For a complete list of available events including text generation, tool usage, lifecycle, and reasoning events, see the [streaming overview](/user-guide/concepts/streaming/index.md#event-types).

> **Note:** For asynchronous applications, consider [async iterators](/user-guide/concepts/streaming/async-iterators/index.md) instead.
