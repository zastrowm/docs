A Graph is a deterministic directed graph based agent orchestration system where agents, custom nodes, or other multi-agent systems (like [Swarm](/user-guide/concepts/multi-agent/swarm/index.md) or nested Graphs) are nodes in a graph. Nodes are executed according to edge dependencies, with output from one node passed as input to connected nodes. The Graph pattern supports both acyclic (DAG) and cyclic topologies, enabling feedback loops and iterative refinement workflows.
...
You can use a [`Graph`](/api/python/strands.multiagent.graph#Graph) or [`Swarm`](/api/python/strands.multiagent.swarm#Swarm) as a node within another Graph:
...
Graphs support remote A2A agents as nodes through the [`A2AAgent`](/user-guide/concepts/multi-agent/agent-to-agent/index.md#a2aagent-consuming-remote-agents) class. You can add it directly to a graph just like a local agent. This enables distributed architectures where orchestration happens locally while specialized tasks run on remote services.
...
You can create custom node types by extending [`MultiAgentBase`](/api/python/strands.multiagent.base#MultiAgentBase) to implement deterministic business logic, data processing pipelines, and hybrid workflows.
...
Graphs support multi-modal inputs like text and images using [`ContentBlocks`](/api/python/strands.types.content#ContentBlock):
...
You can also execute a Graph asynchronously by calling the [`invoke_async`](/api/python/strands.multiagent.graph#Graph.invoke_async) function:
...
Graphs support real-time streaming of events during execution using [`stream_async`](/api/python/strands.multiagent.graph#Graph.stream_async). This provides visibility into node execution, parallel processing, and nested multi-agent systems.
...
See the [streaming overview](/user-guide/concepts/streaming/index.md#multi-agent-events) for details on all multi-agent event types.

## Graph Results

When a Graph completes execution, it returns a [`GraphResult`](/api/python/strands.multiagent.graph#GraphResult) object with detailed information:
...
For detailed information about shared state, including examples and best practices, see [Shared State Across Multi-Agent Patterns](/user-guide/concepts/multi-agent/multi-agent-patterns/index.md#shared-state-across-multi-agent-patterns).

## Graphs as a Tool

Agents can dynamically create and orchestrate graphs by using the `graph` tool available in the [Strands tools package](/user-guide/concepts/tools/community-tools-package/index.md).
