...
3.  `removed_message_count`: This attribute is tracked by conversation managers, and utilized by [Session Management](/user-guide/concepts/agents/session-management/index.md) to efficiently load messages from the session storage. The count represents messages provided by the user or LLM that have been removed from the agent’s messages, but not messages included by the conversation manager through something like summarization.
    
4.  `register_hooks` (optional): Override this method to integrate with [hooks](/user-guide/concepts/agents/hooks/index.md). This enables proactive context management patterns, such as trimming context before model calls. Always call `super().register_hooks` when overriding.
...
(( tab "TypeScript" ))
In TypeScript, conversation managers don’t have a base interface. Instead, they are simply [HookProviders](/user-guide/concepts/agents/hooks/index.md) that can subscribe to any event in the agent lifecycle.
